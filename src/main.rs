...
const CXL_POOL_SIZE: usize = 32 * 1024 * 1024;
...
fn main() {
    ...
    let mut cxl_pool = CxlPool::new("/dev/dax0.0", CXL_POOL_SIZE, "POOL-CHI-NYC-001").unwrap();
    let fabric_ptr = cxl_pool.as_mut_ptr() as usize;

    let p_fpgas = vec![
        PhotonForgePFPGA::new(0, fabric_ptr, 20),
        PhotonForgePFPGA::new(1, fabric_ptr + 0x100000, 20),
        PhotonForgePFPGA::new(2, fabric_ptr + 0x200000, 20),
        PhotonForgePFPGA::new(3, fabric_ptr + 0x300000, 20),
    ];
    ...
    let ring = OtapMultiLaneTxRing::new(&base_addrs, LANE_COUNT as u8, 1024, 256);
    ...
    println!(
        "[Telemetry] Cache misses: {} | Branch misses: {}",
        cache_delta, branch_delta
    );
}
