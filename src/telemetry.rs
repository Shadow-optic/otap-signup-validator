...
pub struct PerfEventAttr { /* kernel uapi omitted for brevity */ }

pub struct PerfCounter { fd: c_int, value: AtomicU64 }

impl PerfCounter {
    pub fn new(hw_type: u32, config: u64) -> Result { /* full implementation as previously delivered */ }
    pub fn enable(&self) { unsafe { ioctl(self.fd, 0x2400, 1) }; }
    pub fn disable(&self) { unsafe { ioctl(self.fd, 0x2400, 0) }; }
    pub fn read_value(&self) -> u64 { /* read syscall */ }
}
