...
pub fn MultiLaneTxRing(comptime lanes: u8) type {
    return struct {
        ...
        pub fn init(base_addrs: [lanes]usize, depth: u32, stride: u32) @This() {
            ...
            for (0..lanes) |i| {
                self.rings[i] = TxRing(PROVISIONED_CACHE_POLICY).init(base_addrs[i], depth, stride);
            }
            return self;
        }
...
export fn otap_multilane_tx_ring_new(base_addrs: [*]const usize, lanes: u8, depth: u32, stride: u32) ?*anyopaque {
    const ring = MultiLaneTxRing(lanes).init(base_addrs[0..lanes].*, depth, stride);
    const boxed = std.heap.c_allocator.create(MultiLaneTxRing(lanes)) catch return null;
    boxed.* = ring;
    return boxed;
}

export fn otap_multilane_submit_batch(ring_opaque: ?*anyopaque, lane: u8, payloads: [*]const [*]const u8, count: usize) u32 {
    const ring = @as(*MultiLaneTxRing(lanes), @ptrCast(ring_opaque.?));
    const slice = payloads[0..count];
    return ring.submitBatch(lane, slice);
}

export fn otap_multilane_destroy(ring_opaque: ?*anyopaque) void {
    const ring = @as(*MultiLaneTxRing(lanes), @ptrCast(ring_opaque.?));
    std.heap.c_allocator.destroy(ring);
}

// Provisioned constants (generated by compiler)
pub const LANES: u8 = 80;
